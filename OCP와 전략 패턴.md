...
* 기능이 늘어날 수록 코드의 길이가 증가되면서 가독성이 안 좋아진다.     
* 또한, 코드를 추가하거나 수정할 위치를 찾는데 점점 오랜 시간이 걸린다.
* `if-else`를 추가하면 수쟁해야 할 코드가 있을 수 있고 수정 작업을 누락할 수 있는 문제점이 생긴다.     
...
* **확장에 대해서는 개방**돼야 하지만 **변경에 대해서는 폐쇄**되어야 한다.           
