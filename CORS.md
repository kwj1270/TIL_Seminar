...
* 서버 : 요청에 대한 응답을 할 때 응답 헤더의 `Access-Control-Allow-Origin`필드 값에     
`서버의 리소스를 접근하는 것이 허용된 출처`를 넣어서 보냅니다.     
* 이후, 요청의 `Origin`과 응답의 `Access-Control-Allow-Origin`을 비교하여 이 응답이 유효한 응답인지 아닌지를 결정합니다.
